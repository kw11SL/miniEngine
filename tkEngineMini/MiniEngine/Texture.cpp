...
void Texture::InitFromDDSFile(const wchar_t* filePath,bool isCubeMap)
{
	//DDSファイルからテクスチャをロード。
	LoadTextureFromDDSFile(filePath,isCubeMap);
...
void Texture::LoadTextureFromDDSFile(const wchar_t* filePath,bool isCubeMap)
{
	auto device = g_graphicsEngine->GetD3DDevice();
	DirectX::ResourceUploadBatch re(device);
	re.Begin();
